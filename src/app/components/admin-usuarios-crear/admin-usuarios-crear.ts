...
export class AdminUsuariosCrear {
  ...
  guardarUsuario(): void {
    ...
    if (!nombre || !correo || !contrasena) {
      this.ngZone.run(() => Swal.fire('Campos obligatorios', 'Nombre, correo y contraseña son requeridos', 'warning'));
      return;
    }

    const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    if (!emailRegex.test(correo)) {
      this.ngZone.run(() => Swal.fire('Correo inválido', 'Ingresa un correo electrónico válido', 'warning'));
      return;
    }

    if (contrasena.length < 6) {
      this.ngZone.run(() => Swal.fire('Contraseña muy corta', 'La contraseña debe tener al menos 6 caracteres', 'warning'));
      return;
    }
    ...
    this.usuarioService.crear({ nombre, correo, contrasena, rol } as Usuario).subscribe({
      next: () => {
        ...
        this.ngZone.run(() => Swal.fire('Éxito', 'Usuario creado correctamente', 'success'));
